...
(defn trip-graphs
  ...
  (let [trip (get (json/read-str trip-json-string) "visits")]
    (html
     [:div#summary.small-text-center {:style "padding-bottom:22px;"}
      [:h1.small-text-center "Time Per Page"]
      [:div#chart-container {:style "width:100%;height:430px;"}]
      [:h4.small-12.columns.medium-8.large-6.medium-centered {:style "border-top:8px solid #eee;padding-top:30px;"} "You started out at"]
      [:h2 (parse-title-from-url (get (first trip) "url"))]
      [:p "on " (pretty-date (get (first trip) "time_visited")) "."]
      [:h4 "and wound up at:"]
      [:h2 (parse-title-from-url (get (last trip) "url"))]
      [:p "on " (pretty-date (get (last trip) "time_visited")) "."]
      [:div#secret-data {:style "display:none;"} trip-json-string]]
     [:div#bye-bye.small-text-center.small-12.columns.medium-8.large-6.medium-centered {:style "border-top:8px solid #eee;padding-top:30px;"}
      [:h1 {:style "margin-bottom:40px;text-style:italic;"} "What a Wikitrip!"]
      [:a.button.large
       {:style "margin-bottom:80px;"
        :href (str "http://twitter.com/share?text=Check out my Wikitrip! @kelseyinnis @depaysant&url=http://wikihole.clojurecup.com/trip/" id "/view")
        :target "_blank"}
       "Share on Twitter!"]])))
